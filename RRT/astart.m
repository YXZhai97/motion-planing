...
% R. Kala (2014) Code for Robot Path Planning using Rapidly-exploring Random Trees, Indian Institute of Information Technology Allahabad, Available at: http://rkala.in/codes.html
...
while failedAttempts<=maxFailedAttempts  % loop to grow RRTs
    if rand < 0.5 %rand returns a uniform distributed number between (0,1)
        sample=rand(1,2) .* size(map); % random sample(element wise multiplication)
    else
